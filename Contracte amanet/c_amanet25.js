...
function ON_LOCATE() {
	...
	if (itsMe) {
		...
		//comm1(0, id, INST.FROMDATE, INST.TRDR, INST.TRDR_CUSTOMER_NAME);
		debugger;
		X.SETPARAM('mod',0);
		X.SETPARAM('id',id);
		X.SETPARAM('fromdate',INST.FROMDATE);
		X.SETPARAM('trdr',INST.TRDR);
		X.SETPARAM('trdrName',INST.TRDR_CUSTOMER_NAME);
		X.EXEC('XCMD:FormImport,ScriptName:Test')
		itsMe = false;
	}
}
...
function ON_SFVPAYTYPE_ACCEPT() {
	INST.CCCPAYTYPE = CCCVPAY.PAYTYPE;
	gridRO(INSTLINES, 0);
		...
		if (DsCateg.mtrcategory == 106 || DsCateg.mtrcategory == 107) {
			...
			while (!INSTLINES.Eof) {
				...
				if (INST.CCCACOMM > 0) {
					...
					DsComm = X.GETSQLDATASET('select top 1 commprc as commprc, commprcr as commprcr, commprci as commprci, cccacomm from cccacomms where branch=' + INST.BRANCH + ' and mtrgroup=' + DsGroup.mtrgroup +
...
function aplicare_comision_adaos() {
	...
	if (DsTipDurata.type == 2) {
		...
		if (INST.CCCACOMM > 0)
			DsMinim = X.GETSQLDATASET('select top 1 isnull(minval,0) as minval from cccacomm where cccacomm=' + INST.CCCACOMM, null); // minim valoare
		else
			DsMinim = X.GETSQLDATASET('select top 1 isnull(minval,0) as minval from cccacomm where branch=' + INST.BRANCH + ' and convert(varchar(10),fromdate,112)<=' + dataCntr + '  order by fromdate desc', null); // minim valoare
...
/*
CREATE TABLE CCCPDFINST(
CCCPDFINST INT NOT NULL PRIMARY KEY IDENTITY(1,1),
INST INT NOT NULL,
ORIG INT,
...
function comm1(mod, id, fromdate, trdr, trdrName) {
	var t1 = new Date().getTime();
	if (!tipInst) {
		xx();
	}

	var sendJson = (function (inst, url, fromdate, trdr, trdrName, b64, pdf) {
		//debugger;
		try {
			var xmlhttp = new ActiveXObject("MSXML2.XMLHTTP.6.0");
			var dataToSend = composeJsonDocs(b64, inst, fromdate, trdr, trdrName);
			...
			var diff = t2 - t1;
			X.WARNING('sent data: ' + diff.toString());
	...
	var callback = (function (xmlhttp, xx, file) {
	...
	//INST.FROMDATE, INST.TRDR, INST.TRDR_CUSTOMER_NAME
	var composeJsonDocs = (function (f64, inst, fromdate, trdr, trdrName) {
		//debugger;
		var doc = '',
		subiect = '';
		...
		var unescapedStr = '{' +
			...
			'"data_docs":"' + X.FORMATDATE('yyyymmdd', fromdate) + '",' +
			'"TRDR":"' + trdr + '",' +
			'"name":"' + trdrName + '",' +
			'"email":"' + cusEmail + '",' +
	...
	var printToPDF = (function (modul, findoc, tipartitura) {
		//tiparire
		var t1 = new Date().getTime();
		var ret = false,
		...
		var t2 = new Date().getTime();
		var diff = t2 - t1;
		X.WARNING('print pdf: ' + diff.toString());

		return ret;
	});

	var convBinTob64 = (function convBinTob64(filePath) {
	...
	var ante = X.GETSQLDATASET('select TOP 1 isnull(INST, 0) INST, BASE64 from CCCPDFINST where isnull(INST, 0)=' + id +
	...
	if (ante.RECORDCOUNT) {
		if (mod == 0) {
			...
			sendJson(id, urlEmail, fromdate, trdr, trdrName, ante.BASE64);
			return;
		} else if (mod == 2) {
			//am info in tabel dar vreau sa o refac
			X.RUNSQL('delete from CCCPDFINST where INST=' + id, null);
		}
	}
	...
	if (pdf) {
		...
		var q = "INSERT INTO CCCPDFINST (INST, ORIG, BASE64, SENTDATE, FROMDATE) VALUES (" + id + "," +
			INST.CCCINST + ",'" + b64 + "','" + formatDate(new Date()) + "', '" + X.FORMATDATE('yyyymmdd', fromdate) + "')";
		X.RUNSQL(q, null);
	}

	//trimte spre site
	sendJson(id, urlEmail, fromdate, trdr, trdrName, b64, pdf);

	var t2 = new Date().getTime();
	var diff = t2 - t1;
	X.WARNING('comm1(): ' + diff);
}
